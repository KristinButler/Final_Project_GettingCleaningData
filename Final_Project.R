...
# Combine data from test and train in one table
...
all_data_meanstd_Q2 <- select(all_data, x = Vmeanstd)
...
label <- mapvalues(x=all_data$test, c("1", "2", "3", "4", "5", "6"), c("LAYING", "SITTING", "STANDING", "WALKING", "WALKING_DOWNSTAIRS", "WALKING_UPSTAIRS"))
all_data$test <- label
...
names(all_data) <- Vfeatures 
names(all_data)[562] <- "test"
names(all_data)[563] <- "subject"
names(all_data)
all_data

#Write the file for submission
write.csv(all_data, file = "data_descript_Q4.csv")
write.table(all_data, file = "data_descript_Q4.txt")
...
all_data_groups <- group_by(all_data, test, subject)
all_data_groups<- select(all_data, test, subject, everything())
all_data_groups
